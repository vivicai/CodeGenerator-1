$(function () {
    ...
    layui.use(['table', 'layer', 'form', 'laypage', 'laydate'], function () {
        ...
        //templet: '#zq_formatter'  解决对象格式化问题
            {field: 'id', title: '编号', sort: true,edit: 'text'},
            ...
            {field: 'userHeadUrl', title: '头像', sort: true,edit: 'text'},
                        ...
                        layer.alert('您确认要删除'+data.length+'条数据吗？', {
                            ...
                            ,yes:function(){
                                ...
                                for (var i=0;i<data.length;i++){
                                    ...
                                    //发送请求到后台
                                    $.post("user/delete", { id: data[i].id }, function (result) {
            ...
            switch(layEvent){
                ...
                case 'del':
                    layer.confirm('您确定删除id：'+data.id+'的数据吗？', function(index){
                        ...
                        $.post("user/delete", { id: data.id }, function (ret) {
                ...
                case 'edit':
                    console.debug(data);
                    zq_form('编辑user','url这个值不管',500,400);
                    //数据回显
                    $("#zq_form").setForm({
                                    id:data.id,
        ...
        form.on('submit(zq_submit)', function(data){
            var formData = data.field;
                    var id = formData.id;//编号
                    var username = formData.username;//用户名
                    var password = formData.password;//密码
                    var sex = formData.sex;//性别
                    var userHeadUrl = formData.userHeadUrl;//头像
                    $.ajax({
                type: "post",  //数据提交方式（post/get）
                url: "/user/save",  //提交到的url
                data: {
                    "id":id,
                    "username":username,
                    "password":password,
